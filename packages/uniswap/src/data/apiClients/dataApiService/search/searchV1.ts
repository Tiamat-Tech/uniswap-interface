 ...
 */
export function useSearchV1Query<TSelectType>({
  input,
  enabled = true,
  select,
}: {
...
export function chainTokenToCurrencyInfo(
  ...
  const safetyInfo = parseCurrencySafetyInfo(
    chainToken.safetyLevel || multichainToken.safetyLevel,
    chainToken.protectionInfo ?? multichainToken.protectionInfo,
  )
