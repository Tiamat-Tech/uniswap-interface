...
export const DISCLOSURES: readonly DisclosureParagraph[] = [
  ...
  [
    'The following parameters are hard-coded into the Products’ routing software (the “Auto Router”): (i) estimation of gas costs via simulation (which may deprioritize high-network fee routes);  (ii) estimation of slippage via simulation (which will revert a transaction that does not meet the user’s maximum specified slippage amount or better); (iii) pool liquidity, as measured by the total value locked across liquidity sources (which will deprioritize or exclude pools falling below a specified threshold); (iv) the maximum number of times a user’s swap can be “split” to allow the router to evaluate whether splitting the user’s swap among multiple liquidity sources may result in a better price; (v) the maximum number of “hops,” meaning potential sequences of digital asset conversions that end in the user receiving its desired output token; and (vi) the maximum number of potential routing paths that the Auto Router algorithm will evaluate in generating a proposed route. The Auto Router does not explore pools with fees greater than 11% as a measure to protect users from unexpected price impact due to unusually high LP fees. These routing-software parameters are not user-configurable transaction-level default parameters. The Products’ routing software applies these routing parameters in a uniform manner and the code can be viewed ',
