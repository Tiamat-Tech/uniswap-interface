...
describe('rowKeys', () => {
  ...
  it('builds a section-scoped item key from the item key', () => {
    expect(getSectionItemRowKey({ sectionRowId: 'stocks', itemKey: 'rwa-collection-NVDA', index: 2 })).toBe(
      'item-stocks-rwa-collection-NVDA',
    )
  })

  it('falls back to the positional index only when the item key is missing', () => {
    expect(getSectionItemRowKey({ sectionRowId: 'stocks', itemKey: undefined, index: 2 })).toBe('item-stocks-2')
  })
