/**
 * Deterministic CSS normalization: compiled declarations are reduced to a
 * canonical longhand `property → value` map — resolve vars → expand
 * shorthands → canonicalize values. Anything unrecognized throws instead of
 * being coerced.
 */
 ...
 * Unknown variables throw unless a fallback exists — callers must
 * never read a result built on unresolved var() references.
 */
...
function hexToRgba(hex: string): Rgba {
  ...
  if (hex.length === 3 || hex.length === 4) {
    return {
      r: expand(hex.charAt(0)),
      g: expand(hex.charAt(1)),
      b: expand(hex.charAt(2)),
      a: hex.length === 4 ? expand(hex.charAt(3)) / 255 : 1,
    }
  }
...
function rgbFunctionToRgba(inner: string): Rgba {
  ...
  return { r: channel(red), g: channel(green), b: channel(blue), a }
}
...
function expandFlex(value: string): Declarations {
  ...
  return {
    'flex-grow': parts[0] ?? '',
    'flex-shrink': (parts.length > 1 ? parts[1] : '1') ?? '1',
    'flex-basis': (parts.length > 2 ? parts[2] : '0%') ?? '0%',
  }
}
...
export function expandDeclaration(prop: string, value: string): Declarations {
  const expansion = SHORTHAND_EXPANSIONS[prop]
