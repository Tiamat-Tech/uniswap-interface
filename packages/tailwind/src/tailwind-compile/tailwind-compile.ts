/**
 * Compiles candidate Tailwind classes to CSS declarations using the real
 * Tailwind v4 Node API against the exact stylesheet stack the web app uses
 * (`@import "tailwindcss"; @import "@universe/tailwind/tailwind";` — see
 ...
 * so a class's declarations can be resolved per scope and per theme.
 * `@keyframes` bodies are parsed too, so animation-preset consumers can read
 * the actual frame declarations.
 */
...
function collectRootVars({ rules, table, selectorFilter }: CollectVarsOptions): void {
  for (const rule of rules) {
    ...
    if (rule.selector.startsWith('@property ')) {
      ...
      const initialValue = rule.declarations['initial-value']
      if (Object.hasOwn(rule.declarations, 'initial-value') && !table.has(name)) {
        table.set(name, String(initialValue))
      }
      continue
    }
...
export async function compileTailwindClasses(candidates: string[]): Promise<CompiledTailwind> {
  ...
  // cross-package read: a package dep edge tailwind → mycelium would cycle
  // (mycelium already depends on tailwind).
