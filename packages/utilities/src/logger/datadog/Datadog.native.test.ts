...
const addAction = vi.fn((..._args: unknown[]): Promise<void> => Promise.resolve())
const addError = vi.fn((..._args: unknown[]): Promise<void> => Promise.resolve())

vi.mock('@datadog/mobile-react-native', () => ({
  DdLogs: { info: vi.fn(() => Promise.resolve()), warn: vi.fn(() => Promise.resolve()) },
  DdRum: {
    addAction: (...args: unknown[]): Promise<void> => addAction(...args),
    addError: (...args: unknown[]): Promise<void> => addError(...args),
  },
  DdSdkReactNative: { addAttributes: vi.fn(() => Promise.resolve()) },
  ErrorSource: { SOURCE: 'SOURCE' },
  RumActionType: { CUSTOM: 'custom' },
}))

const { createDatadogReduxEnhancer, logErrorToDatadog } = await import('utilities/src/logger/datadog/Datadog.native')
...
function unlockActionContext(): { type: string; payload?: unknown; payloadScrubFailed?: true } | undefined {
  return addAction.mock.calls.find(([, name]) => name === 'Redux Action: auth/trigger')?.[2] as
    | { type: string; payload?: unknown; payloadScrubFailed?: true }
    | undefined
}

describe('createDatadogReduxEnhancer (native)', () => {
  beforeEach(() => {
    addAction.mockClear()
    addError.mockClear()
  })
  ...
  it('drops the cached state snapshot when consent is withdrawn mid-session', () => {
    ...
    expect(addError.mock.calls.at(-1)?.[3]).toEqual({ ...context, reduxState: { transactions: { pending: [] } } })

    allowAnalytics = false
    store.dispatch({ type: 'telemetry/setAllowAnalytics' })
    logErrorToDatadog(new Error('after opt-out'), context)
    expect(addError.mock.calls.at(-1)?.[3]).toEqual({ ...context, reduxState: undefined })
  })
})
