...
describe('useGetPoolTokenPercentage', () => {
  ...
  // A zero total supply satisfies `totalSupply >= liquidityAmount` (both are then 0), and the
  // resulting `new Percent(0, 0)` throws "[big.js] Division by zero" the first time a consumer
  // calls toFixed() on it (RemoveLiquidityReview, IncreaseLiquidityReview).
  it('returns undefined if totalSupply.quotient is zero', () => {
    const { result } = renderHook(() =>
      useGetPoolTokenPercentage({
        liquidityAmount: { quotient: JSBI.BigInt(0) },
        totalSupply: { quotient: JSBI.BigInt(0) },
      } as any),
    )
    expect(result.current).toBeUndefined()
  })
